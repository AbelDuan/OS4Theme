...
public class SettingsActivity extends Activity {
    ...
    private void buildUi() {
        ...
        // 隐藏通知「清除通知」按钮背景圆圈和叉叉：启用/不启用 单选
        TextView notifClearHead = new TextView(this);
        notifClearHead.setText("隐藏通知清除按钮");
        notifClearHead.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
        notifClearHead.setTypeface(Typeface.DEFAULT_BOLD);
        notifClearHead.setTextColor(Color.parseColor("#222222"));
        LinearLayout.LayoutParams lpNotifClearHead = mw();
        lpNotifClearHead.topMargin = dp(10);
        card.addView(notifClearHead, lpNotifClearHead);

        final RadioGroup rgNotifClear = new RadioGroup(this);
        rgNotifClear.setOrientation(RadioGroup.VERTICAL);
        final RadioButton notifClearOn = radio("启用");
        final RadioButton notifClearOff = radio("不启用");
        rgNotifClear.addView(notifClearOn, mw());
        rgNotifClear.addView(notifClearOff, mw());
        if (sp().getBoolean(Constants.PREFS_HIDE_NOTIF_CLEAR,
                Constants.DEFAULT_HIDE_NOTIF_CLEAR)) {
            notifClearOn.setChecked(true);
        } else {
            notifClearOff.setChecked(true);
        }
        sInit = false;
        rgNotifClear.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if (sInit) return;
                sp().edit().putBoolean(Constants.PREFS_HIDE_NOTIF_CLEAR,
                        checkedId == notifClearOn.getId()).commit();
                toast("已保存（重启系统界面后生效）");
            }
        });
        card.addView(rgNotifClear, mw());

        // 隐藏桌面多任务「清理任务」按钮背景圆圈和叉叉：启用/不启用 单选
        TextView recentsClearHead = new TextView(this);
        recentsClearHead.setText("隐藏多任务清理任务按钮");
        recentsClearHead.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
        recentsClearHead.setTypeface(Typeface.DEFAULT_BOLD);
        recentsClearHead.setTextColor(Color.parseColor("#222222"));
        LinearLayout.LayoutParams lpRecentsClearHead = mw();
        lpRecentsClearHead.topMargin = dp(10);
        card.addView(recentsClearHead, lpRecentsClearHead);

        final RadioGroup rgRecentsClear = new RadioGroup(this);
        rgRecentsClear.setOrientation(RadioGroup.VERTICAL);
        final RadioButton recentsClearOn = radio("启用");
        final RadioButton recentsClearOff = radio("不启用");
        rgRecentsClear.addView(recentsClearOn, mw());
        rgRecentsClear.addView(recentsClearOff, mw());
        if (sp().getBoolean(Constants.PREFS_HIDE_RECENTS_CLEAR,
                Constants.DEFAULT_HIDE_RECENTS_CLEAR)) {
            recentsClearOn.setChecked(true);
        } else {
            recentsClearOff.setChecked(true);
        }
        sInit = false;
        rgRecentsClear.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if (sInit) return;
                sp().edit().putBoolean(Constants.PREFS_HIDE_RECENTS_CLEAR,
                        checkedId == recentsClearOn.getId()).commit();
                toast("已保存（重启系统界面和桌面后生效）");
            }
        });
        card.addView(rgRecentsClear, mw());
